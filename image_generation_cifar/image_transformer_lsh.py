...
from quantized_qk_reformer import QuantizedQKLSHSelfAttention
...
class ImageTransformer(nn.Module):
    ...
    def forward(self, X, sampling=False):
        ...
        if self.hparams.distr == "dmol":
            ...
            X = F.relu(self.embedding_conv(X.cuda()).cuda())
        ...
        # logging.info("after dropout+" + str(X.shape))
    ...
    def sample(self, n, device, argmax=False):
        ...
        for curr_infer_length in tqdm(range(total_len)):
            # logging.info("sample:" + str(samples.shape) +", "+str(curr_infer_length))
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, hparams):
        super().__init__()
        # self.attn = LSHSelfAttention(dim=hparams.hidden_size, causal = True, use_full_attn=True)
        # self.attn = LSHSelfAttention(dim=hparams.hidden_size, bucket_size = 32, 
        #     n_hashes = 8,
            # dropout = 0.1,
            # causal = True,
